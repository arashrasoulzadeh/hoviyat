...
//
 ...
 type Config struct {
 	...
 	TenantSignupRateLimit int
 	TenantSignupWindow    time.Duration
 	RedisURL              string
 ...
 func Load() Config {
 	return Config{
 		...
 		TenantSignupRateLimit: 5,
 		TenantSignupWindow:    time.Hour,
 		RedisURL:              getEnv("HOVIYAT_REDIS_URL", "redis://localhost:6379"),
 		AuthzCacheTTL:         5 * time.Minute,
 	}
 }

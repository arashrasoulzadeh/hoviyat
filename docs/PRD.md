...
- Consent screens for OAuth2/OIDC flows where Hoviyat acts as an identity provider to third-party apps: explicitly lists the requested scopes/claims in plain language (e.g. "this app wants: your email, your role, your organization") before the user approves. Consent is remembered per app after the first grant — subsequent logins to the same app skip the consent screen unless requested scopes change. A "connected apps" list in profile settings lets the user review and revoke a previously-granted app's access at any time (revoking invalidates that app's tokens and forces consent again on next login).
- Cross-app login behavior: logging into Site A does **not** silently log the user into Site B, even if both are registered client apps of the same tenant — each app's login always re-prompts for credentials (no shared/silent Hoviyat session across client apps). Only the consent screen (not the credential prompt) is remembered per app, per the bullet above. This is a deliberate choice against typical "Universal Login" SSO behavior (e.g. Auth0/Google-style silent cross-app sessions), trading convenience for a simpler, more explicit security model.
